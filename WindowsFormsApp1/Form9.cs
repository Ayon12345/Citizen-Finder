...
namespace WindowsFormsApp1
{
    public partial class Form9 : Form
    {
        ...
        {
            string query = "SELECT * FROM `student` WHERE CONCAT(`FirstName`, `LastName`, `Email`, `Phone`, `StudentId`, `School`, `Class`, `DateOfBirth`, `NationalId`, `BloodGroup`, `Age`, `Gender`) like '%" + valueToSearch + "%'";
            command = new MySqlCommand(query, connection);
            adapter = new MySqlDataAdapter(command);
            table = new DataTable();
            adapter.Fill(table);
        ...
        {
            string query = "SELECT * FROM `information` WHERE CONCAT(`FirstName`, `LastName`, `Email`, `Phone`, `PoliceId`, `Region`, `Rank`, `DateOfBirth`, `NationalId`, `BloodGroup`, `Age`, `Gender`) like '%" + valueToSearch + "%'";
            command = new MySqlCommand(query, connection);
            adapter = new MySqlDataAdapter(command);
            table = new DataTable();
            adapter.Fill(table);
